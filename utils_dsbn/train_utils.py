...
def adaptation_factor(i_iter, warm_step, max_step,  gamma=10):
    if i_iter > warm_step:
        p = (i_iter - warm_step) / float(max_step)
        p = max(min(p, 1.0), 0.0)
...
def lr_poly(base_lr, i_iter, alpha=10, beta=0.75, num_steps=250000):
    if i_iter == 0:
        ...
        adjust_lr = base_lr / (((1 + alpha * float(i_iter) / num_steps) ** (beta)))
...
class LRScheduler:
    ...
    def current_lr(self, i_iter):
        if i_iter < self.warmup_steps:
            ...
            lr_i_iter = max(i_iter - self.warmup_steps, 0)
            self_adjust_lr = lr_poly(self.learning_rate, lr_i_iter, alpha=self.alpha, beta=self.beta, num_steps=self.num_steps)
            return self_adjust_lr
